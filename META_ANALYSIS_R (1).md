...
This workflow runs a **random + fixed effects meta-analysis** for odds ratios and saves standard plots.

## Input data formats

Your CSV can be in any of these formats:
...
### Option B: `OR` + CI bounds
Required columns:
- `or`
- `ci_lower`
- `ci_upper`

### Option C: `OR` + a CI string column
Required columns:
- `or`
- a CI column like `x95_percent_ci` containing values like `1.20-1.80` (also accepts en-dash `1.20–1.80`)

Optional columns (used for labels):
- `label` **or** `gene` + `snp`
